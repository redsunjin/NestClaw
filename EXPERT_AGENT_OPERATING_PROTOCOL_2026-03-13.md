...
- `Plan` 단계에는 A01의 범위 고정과 함께 A03의 `AI-First Planner Design` 검토가 포함된다.

## 전문가 에이전트 역할
| 단계 | 담당 에이전트 | 책임 |
|---|---|---|
| Plan | A01 Product Planner + A03 LLM Orchestrator | 범위, 비범위, acceptance, risk, test plan + AI-first planner design 고정 |
...
## 표준 절차
1. `A01`이 MWU 범위를 선언한다.
2. `A03`가 같은 Plan 패키지 안에서 `AI-First Planner Design`을 고정한다.
3. `A04`가 구현 전 보안/정책 검토를 끝낸다.
4. `A02`가 구현 후 implement notes를 채운다.
5. `A06`가 `run_micro_cycle`과 QA worktree 검증으로 evaluate를 닫는다.
6. `A09`가 commit/push/QA sync/evidence 반영까지 마무리한다.
...
## Priority Campaign Layer
- 여러 우선순위를 멈추지 않고 이어가야 할 때는 MWU 위에 Priority Campaign을 둔다.
- Campaign은 `work/priority_campaigns/<campaign_id>/campaign.json`으로 관리한다.
- 각 campaign item은 정확히 하나의 MWU를 가리키고, 상태는 `pending / in_progress / completed`만 허용한다.
- `A09`는 sync가 끝난 item을 campaign에서 `completed`로 마킹하고, 다음 pending item을 즉시 준비한다.
- stop condition이 없으면 다음 item 준비에서 멈추지 않는다.
- stop condition은 [PRIORITY_CAMPAIGN_PROTOCOL_2026-03-15.md](/Users/Agent/ps-workspace/NestClaw_works/worktrees/nestclaw-ideation/PRIORITY_CAMPAIGN_PROTOCOL_2026-03-15.md)를 따른다.
...
## 다음 담당자 판정 규칙
- Plan gate 미완료: `A01 Product Planner + A03 LLM Orchestrator`

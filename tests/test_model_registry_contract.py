...
class TestModelRegistryContract(unittest.TestCase):
    def test_load_model_registry_from_yaml(self) -> None:
        ...
        self.assertEqual(len(registry.routing_rules), 6)
    ...
    def test_selects_local_provider_for_incident_planning(self) -> None:
        registry = load_model_registry()

        selection = select_provider(registry, sensitivity="low", task_type="incident_plan_actions", external_send=False)

...
class TestAgentPlannerContract(unittest.TestCase):
    ...
    def test_fallback_can_plan_summary_and_slack_when_notification_is_requested(self) -> None:
        with patch.dict(os.environ, {"NEWCLAW_ENABLE_LLM_PLANNER": "0"}, clear=False):
            result = self.planner.plan_task_actions(
                request_text="운영회의를 요약하고 슬랙으로 공유해줘",
                task_input=self.task_input,
                metadata={"notify_channel": "#ops-alerts"},
                available_tools=[SUMMARY_TOOL, SLACK_TOOL],
                sensitivity="low",
                external_send=False,
                default_notify_channel="#ops-alerts",
            )

        self.assertEqual([item.tool_id for item in result.actions], ["internal.summary.generate", "slack.message.send"])
        self.assertEqual(result.actions[1].payload_overrides["channel"], "#ops-alerts")
        ...
        with (
            ...
            patch(
                ...
                return_value=(
                    ...
                    '],"confidence":0.92,"rationale":"two-step plan"}'
                ),
            ),
        ):
            result = self.planner.plan_task_actions(

...
class TestStage8Contract(unittest.TestCase):
    ...
    def test_micro_workflow_assets_exist(self) -> None:
        ...
        self.assertTrue(Path("app/agent_planner.py").is_file())
        ...
        self.assertTrue(Path("tests/test_agent_planner_contract.py").is_file())
        self.assertTrue(Path("tests/test_model_registry_runtime.py").is_file())
        self.assertTrue(Path("tests/test_agent_planner_runtime.py").is_file())
    ...
    def test_dev_qa_cycle_supports_stage8(self) -> None:
        ...
        self.assertIn("tests.test_agent_planner_runtime", source)
    ...
    def test_main_uses_orchestration_service_layer(self) -> None:
        ...
        self.assertIn("TASK_PLANNER = AgentPlanner(MODEL_REGISTRY)", source)
    ...
    def test_provider_invoker_is_connected_to_summary_runtime(self) -> None:
        ...
        planner_source = Path("app/agent_planner.py").read_text(encoding="utf-8")
        self.assertIn("PROVIDER_INVOKER = ProviderInvoker()", main_source)
        self.assertIn("TASK_PLAN_GENERATED", main_source)
        self.assertIn("planning_provenance", main_source)
        self.assertIn("planning_provenance", service_source)
    ...
    def test_web_console_is_exposed_from_root(self) -> None:
        ...
        self.assertIn("data-approval-detail", js_source)

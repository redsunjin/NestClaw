...
목표 제품은 `조직용 orchestration runtime/control plane`이며, 현재 task workflow는 AI-first planning baseline을 기본으로 하고 heuristic/template fallback은 운영 연속성을 위한 `degraded mode`로만 유지한다.
...
- `task` workflow는 현재 LLM planner baseline을 사용하고, 허용 tool set은 `internal.summary.generate`, `redmine.issue.create`, `slack.message.send`다.

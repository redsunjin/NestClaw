# Stage 8 실행안: 첫 번째 vertical - 운영장애 대응 오케스트레이션 + 외부 RAG/MCP 연동

## 문서 목적
NestClaw 전체 제품 목표는 `다양한 도구를 정책적으로 사용하는 실행형 에이전트`로 두고,
이 문서는 그중 첫 번째 high-risk vertical인 `운영장애 대응 자동화`에 대한 실행 기준을 정의한다.
외부 업무 지식 RAG와 MCP 도구(예: Redmine)를 안전하게 결합하는 세부 설계를 다룬다.

## 의사결정 배경 (2026-03-04 컨텍스트 흡수)
- 아이디에이션 후보 3개(S1/S2/S3) 비교 결과, Stage 8 1순위 vertical은 S1(운영장애 대응 오케스트레이션)으로 확정했다.
- S2(배포/변경 위험 사전심사), S3(온콜 핸드오버 자동화)는 S1 공통 자산(승인/정책/감사/RAG/MCP)을 재사용하는 후속 확장으로 분류한다.
- 전문가 운영 체계는 `AGENT_EXPERT_GROUP.md`를 기준으로 유지하며, Stage 8 실행은 Core 6(A01~A06) 주도 + Advisory 2(A07~A08) 검토 구조를 따른다.
- 자동화 경계는 "근거 수집/액션 카드/티켓 반영 자동화 + 고위험 액션 승인 필수" 원칙으로 고정한다.
...
다만 이것은 NestClaw 전체 제품 범위를 고정하는 문서가 아니라, broader execution agent의 첫 번째 vertical 설계 문서다.
